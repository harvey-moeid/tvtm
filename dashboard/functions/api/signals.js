export async function onRequest(context) {
  ...
  if (!env.DB) {
    return new Response(JSON.stringify({ error: "D1 binding 'DB' tidak ditemukan." }), {
      status: 500, headers
    });
  }
  ...
  try {
    const dataSql =
      ...
      "zone_type, zone_level, structure_event, pattern, candle_time, notified, created_at " +
      "FROM signals" + where + " ORDER BY created_at DESC LIMIT ? OFFSET ?";

    const countSql = "SELECT COUNT(*) AS total FROM signals" + where;
    ...
    const statsSql =
      ...
      "MAX(created_at) AS latest FROM signals" + where;
    ...
    const [dataResult, countResult, symbolsResult, statsResult] = await Promise.all([
      ...
      env.DB.prepare(statsSql).bind(...params).first(),
    ]);
    ...
    return new Response(JSON.stringify({
      ...
      limit,
      offset,
      symbols: (symbolsResult.results || []).map((row) => row.symbol),
      stats: {
