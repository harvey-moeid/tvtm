...
def test_buy_hits_tp1_then_tp2():
    d=FakeD1Client(); candles=[make_candle(1,100,106,99,104),make_candle(2,105,111,104,110)]
    assert track_trade_row(d,row(),candles)=="CLOSED_TP2"; assert d.trades[0]["status"]=="CLOSED"
def test_buy_hits_sl():
    d=FakeD1Client(); assert track_trade_row(d,row(),[make_candle(1,100,101,94,96)])=="CLOSED_SL"
def test_same_candle_sl_and_tp_is_conservative_sl():
    d=FakeD1Client(); assert track_trade_row(d,row(),[make_candle(1,100,106,94,100)])=="CLOSED_SL"
