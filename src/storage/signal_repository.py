"""D1 repositories for signal idempotency and trade lifecycle."""
...
def try_reserve(d1:D1Client, signal:Signal)->bool:
    ...
    data=d1.execute(_INSERT_SQL,[
        signal.signal_key,signal.cooldown_key,signal.symbol,signal.market,signal.timeframe,
        signal.direction.value,signal.m15_bias.value,signal.price,signal.zone_type,signal.zone_level,
        signal.structure_event,signal.pattern,signal.candle_time_iso,signal.candle_open_time_ms,
        signal.stop_loss,signal.take_profit_1,signal.take_profit_2,signal.risk_reward_1,signal.risk_reward_2,
        signal.atr,signal.zone_tolerance_pct_used,signal.confidence_pct,signal.score,
        json.dumps([{"label":c.label,"valid":c.valid,"detail":c.detail,"weight":c.weight} for c in signal.checklist],separators=(",",":"))
    ])
    meta=data.get("result",[{}])[0].get("meta",{})
    return bool(meta.get("rows_written",meta.get("changes",0)))

def mark_notified(d1:D1Client,signal_key:str)->None:
    d1.execute("UPDATE signals SET notified=1 WHERE signal_key=?",[signal_key])

def create_trade(d1:D1Client,signal:Signal)->bool:
    ...
    data=d1.execute("""INSERT OR IGNORE INTO trades
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?)""",[
        signal.signal_key,signal.symbol,signal.market,signal.timeframe,signal.direction.value,
        signal.price,signal.stop_loss,signal.take_profit_1,signal.take_profit_2,signal.candle_time_iso
    ])
    meta=data.get("result",[{}])[0].get("meta",{})
    return bool(meta.get("rows_written",meta.get("changes",0)))

def list_open_trades(d1:D1Client)->list[dict]:
...
def update_trade(d1:D1Client,trade_id:int,fields:dict)->None:
    ...
    d1.execute(f"UPDATE trades SET {assignments}, updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?",values)
