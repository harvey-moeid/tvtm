...
def evaluate_market(market_cfg,strategy_cfg,d1,webhook_url=None)->Optional[Signal]:
 symbol=market_cfg['symbol']; exchange=market_cfg['exchange']; exchange_symbol=market_cfg['exchange_symbol']; n=strategy_cfg['minHistoricalCandles']; lookback=strategy_cfg['swingLookback']
 ...
 if webhook_url and tracking['closed_trades']:
  ...
  for trade in tracking['closed_trades']:send_discord_trade_closed(webhook_url,trade)
 ...
 signal=evaluate_m5_trigger(market_cfg['id'],symbol,market_cfg['market'],bias.bias,bias.structure,m15,m5,strategy_cfg,d1)
 if signal is None:return None
 if not try_reserve(d1,signal):return None
 create_trade(d1,signal);return signal
